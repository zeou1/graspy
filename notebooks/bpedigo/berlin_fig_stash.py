...
def evaluate_models(
    ...
    for model, name in zip(models, names):
        ...
        if name in ["ER", "SBM"]:
            ax = scatterplot(
                latent, labels=cell_labels, height=4, alpha=0.6, font_scale=2, size=125
            )
        else:
            ax = scatterplot(
                latent, labels=cell_labels, height=4, alpha=0.6, font_scale=2
            )
        # plt.suptitle(name, y=0.94, x=0.1, fontsize=30, horizontalalignment="left")
        plt.tight_layout(rect=(0.04, 0, 0.95, 0.95))
        plt.savefig(name + "latent.png", format="png", dpi=300)
        plt.close()


ase = AdjacencySpectralEmbed(n_components=2)
latent = ase.fit_transform(left_adj_uw)
name_map = {
    "O": "O - Output neuron",
    "I": "I - MB input neuron",
    "K": "K - Kenyon cell",
    "P": "P - MB projection neuron",
}
from operator import itemgetter

cell_labels = np.array(itemgetter(*cell_labels)(name_map))
...
plt.tight_layout()
plt.savefig("legendn.png", format="png", dpi=300)
plt.close()
